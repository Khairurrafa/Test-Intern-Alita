<x-layout>
    <div class="w-full mx-auto max-w-screen-xl pt-28 md:flex md:items-center md:justify-between">
        <h1 class="text-4xl font-bold text-gray-800">Data Karyawan</h1>
        <div class="flex flex-wrap gap-7 items-center mt-3 text-sm font-medium text-gray-500 dark:text-gray-400 sm:mt-0">
            <form class="max-w-md mx-auto mt-3" method="GET" action="{{ route('employees.index') }}">
                <input type="hidden" name="search" value="{{ request('search') }}" />
                <button type="submit" name="filter_special" value="1" class="box-primary text-orange-500 hover:text-white border border-orange-500 hover:bg-orange-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-semibold text-xl px-10 py-3 text-center me-2 mb-2 dark:border-orange-500 dark:text-orange-500 dark:hover:text-white dark:hover:bg-orange-500 dark:focus:ring-orange-500">Umur > 28 dan di Jakarta</button>
            </form>
            <form class="max-w-md mx-auto" method="GET" action="{{ route('employees.index') }}">
            ...
            <thead class="text-xs text-white uppercase bg-gray-50 dark:bg-orange-400 dark:text-white">
                <tr>
                    <th scope="col" class="px-6 py-3">
                        <a href="{{ route('employees.index', ['sort_by' => 'name', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                            Nama
                            @if($sort_by == 'name')
                                <span>{{ $order == 'asc' ? '▲' : '▼' }}</span>
                            @endif
                    ...
                    <th scope="col" class="px-6 py-3">
                        <a href="{{ route('employees.index', ['sort_by' => 'location_id', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                            Lokasi
                            @if($sort_by == 'location_id')
                                <span>{{ $order == 'asc' ? '▲' : '▼' }}</span>
                            @endif
                        </a>
                    </th>
                    <th scope="col" class="px-6 py-3">
                        <a href="{{ route('employees.index', ['sort_by' => 'birth_date', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                            Tanggal Lahir
    ...
    <div class="max-w-screen-xl mx-auto pt-10 gap-5 flex flex-row items-end justify-end">
        <a href="{{ route('employees.create') }}" class="box-primary text-orange-500 hover:text-white border border-orange-500 hover:bg-orange-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-semibold text-xl px-10 py-3 text-center me-2 mb-2 dark:border-orange-500 dark:text-orange-500 dark:hover:text-white dark:hover:bg-orange-500 dark:focus:ring-orange-500">Tambah Data</a>
    </div>
